...
Sockets.initSocket = async function (params, closeHandler, logger, eventHandler) {
  try {
    ...
    if (Sockets.ws[topic]) {
      logger.info("[WS initSocket] socket for topic " + topic + " already exists, close it and call closeHandler");
      await Sockets.closeSocket(params.topic, logger, closeHandler)
    }
    ...
    ws.on('open', async () => {
      ...
      await Sockets.subscribe(topic, endpoint, type, eventHandler, params.multiplex, logger)
...
Sockets.closeSocket = async function (topic, logger) {
  try {

    if (!topic || !logger) {
      logger.warn("[WS closeSocket] missing args", topic, logger)
      return 
    }

    Sockets.ws[topic].close()
    clearInterval(Sockets.ws[topic].heartbeat)
    return true

  } catch (err) {
    logger.error("[WS closeSocket] error closing socket " + topic + " - " + err);
    return false
  }
} 
...
// subscribe to topic - not the same as connection :)
Sockets.subscribe = async function (topic, endpoint, type, eventHandler, multiplex, logger) {
  try {
    logger.info("[WS sub] Current connections: " + Object.keys(Sockets.ws))
    let ws = Sockets.ws[topic]
    ...
    if (!!multiplex && multiplex.openTunnel) {
      ...
      if (!!multiplex && multiplex.tunnelId) {
