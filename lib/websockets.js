...
*/
Sockets.initSocket = async function (params, closeHandler, eventHandler) {
  try {
    ...
    ws.on('open', () => {
      ...
      Sockets.subscribe(topic, endpoint, type, eventHandler, params.multiplex)
...
Sockets.subscribe = async function (topic, endpoint, type, eventHandler, multiplex, tunnelId) {
  try {
    console.log("Current topics: " + Object.keys(Sockets.ws))
    let ws = Sockets.ws[topic]
    if (ws) {
      console.log("Topic already subscribed, unsubscribing...")

      let unsubMultiplex = null;

      if (!!multiplex) {
        unsubMultiplex = {closeTunnel: true, tunnelId: multiplex.tunnelId ? multiplex.tunnelId : tunnelId}
      }

      const ok = await Sockets.unsubscribe(topic, false, null, eventHandler, unsubMultiplex)
      if (ok) return false
    }
...
Sockets.unsubscribe = async function (topic, endpoint, type, eventHandler, multiplex) {
  try {
    let ws = Sockets.ws[topic]
    if (!ws) {
    ...
    console.log("Unsubscribing topic: " + topic + ", multiplex? " + multiplex)
